...
def process_kicad_pcb_sxp(file_path)
  ...
  collect_nodes_by_key(pcb_tree, "footprint").each do |fp|
    fp_dump_str = fp.flatten.join(" ")
    fp_name = nil
    if fp_dump_str =~ /(INV_X1_M_UD|INV_X1_M|INV_X1_UD|INV_X1)/
      fp_name = $1
    end
    ...
    if fp_name
      ...
      if at_node
        footprints << { fp_name: fp_name, ref: ref, x: at_node.at(1).to_f, y: at_node.at(2).to_f, rot: (at_node.at(3) ? at_node.at(3).to_f : 0.0) }
      end
    end
  end
...
File.open(OUTPUT_RB_PATH, "w") do |f|
  ...
  # デバイス配置 (完璧に合致しているトランスと位置関係を100%完全固定)
  data[:footprints].each do |fp|
    f.puts "\n  # --- #{fp[:ref]} (#{fp[:fp_name]}) ---"
    f.puts "  base_x = #{fp[:x]} / dbu"
    f.puts "  base_y = #{-fp[:y]} / dbu"
  ...
  # 塗りつぶしゾーン（電源レール）
  # 【ズレの完全補正】画像から割り出した電源レール単体の正確な幾何学シフト（Y方向に+3.5mmスライド）を強制適用
  if data[:zones].any?
    f.puts "\n  # --- 電源レール(Zone)描画 ---"
    data[:zones].each_with_index do |zone, index|
      ...
      f.puts "    pts = []"
      zone[:pts].each do |pt|
        # KiCadのZoneの原点不整合を直すため、Y軸に「+3.5mm」のオフセット補正を加えます
        f.puts "    pts << Point.new((#{pt[:x]}/dbu).to_i, (#{-pt[:y] + 3.5}/dbu).to_i)"
      end
...
puts "修正完了: レール（Zone）のシフトバグのみをピンポイントで補正したマクロ [#{OUTPUT_RB_PATH}] を上書き生成しました。"
